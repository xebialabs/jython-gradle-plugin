...
This repository is a fork of the original https://github.com/hierynomus/jython-gradle-plugin[jython-gradle-plugin] created and maintained by Jeroen van Erp (https://github.com/hierynomus[@hierynomus]).

The original plugin was created to bundle Jython/Python libraries in JAR files for Gradle projects. This fork is maintained by Digital.ai with modifications to support internal requirements and updated dependencies.

*Original Author*: Jeroen van Erp (jeroen@hierynomus.com)

*Original Repository*: https://github.com/hierynomus/jython-gradle-plugin

We are grateful to Jeroen van Erp for creating and maintaining the original plugin, which serves as the foundation for this fork.

== Usage
This XebiaLabs fork is published to internal Nexus repository.
...
=== Using the XebiaLabs fork
In your `build.gradle` file add:

[source,groovy,subs="verbatim,attributes"]
----
plugins {
    id "com.xebialabs.jython" version "{jython_plugin_version}"
}
----

=== Legacy setup (Gradle 1.x/2.0)
In your `build.gradle` file add:
...
----
    buildscript {
        ...
        dependencies {
            classpath 'com.xebialabs.gradle.plugins:jython-gradle-plugin:{jython_plugin_version}'
        }
    }
----

=== Using the original plugin
If you prefer to use the original hierynomus version, use:
